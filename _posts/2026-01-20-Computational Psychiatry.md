...
Computational psychiatry sits at the intersection of neuroscience, computer science, and clinical practice, using mathematical models to make sense of mental disorders and guide more precise assessment and intervention. Rather than replacing clinicians, these models offer testable hypotheses about how symptoms arise from learning, perception, and social interaction, from the level of synapses up to populations. Computational psychiatry uses formal models to link observations (symptoms, behavior, brain signals) to underlying cognitive and neural processes. These models can be fit to individual patients, providing quantitative “fingerprints” of mechanisms such as reward sensitivity, belief updating, or social learning.

Key goals include:  
- Explaining how specific symptoms might arise from alterations in learning, valuation, or perception.  
- Improving prediction of illness trajectories and treatment response.  
- Bridging levels of explanation, from neural circuits to behavior and population health.

The field has grown rapidly since the 1990s and now draws on reinforcement learning, partially observable Markov decision processes (POMDPs), inverse reinforcement learning, agent‑based modeling, and predictive coding / Bayesian brain frameworks.

## Reinforcement learning and decision-making

Reinforcement learning (RL) provides a family of models describing how agents learn to select actions based on past rewards and punishments. In these models, the agent updates expectations about outcomes when reality deviates from prediction, producing a reward prediction error signal that has clear analogues in dopaminergic brain activity.

In depression, RL models have been used to formalize anhedonia and motivational deficits as changes in reward sensitivity, learning rate, or beliefs about controllability. For example, some work suggests that depressed individuals may underweight positive feedback and over‑generalize negative experiences, capturing phenomena like learned helplessness as altered value estimates for actions and outcomes. Addiction research uses RL to characterize the shift from goal‑directed (model‑based) control to habitual (model‑free) responding, where drug cues continue to drive behavior despite mounting negative consequences. Models in this area often focus on how drugs distort dopamine‑encoded prediction errors and how this biases learning toward drug‑related actions and cues.

More recent work extends simple model‑free formulations by combining them with latent‑state or latent‑cause inference, allowing the model to represent persistent internal motivational states (such as craving) that shape decisions across contexts. These hybrid models are better suited to capturing phenomena like context‑dependent relapse or sudden shifts in behavior after periods of abstinence.
