...
```{r}
crosstable::crosstable(health_data, gender, by=hiv_status) %>% as_flextable(keep_id=F)
crosstable::crosstable(data_anon_01, gender, by=hiv_status) %>% as_flextable(keep_id=F)
```
